...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
   ...
   if (isset($_FILES["image"]) && $_FILES["image"]["error"] === UPLOAD_ERR_OK) {
      ...
      if (move_uploaded_file($_FILES["image"]["tmp_name"], $filePath)) {
         ...
         if (!empty($fileName) && !empty($book_Id) && !empty($bookName) 
         && !empty($bookPrice) && !empty($language) && !empty($publisher) && !empty($author) 
         && !empty($category) && !empty($publishDate) && !empty($stockedBook)) {
            $query = "INSERT INTO book (ISBN_No, image_name, image_url, Book_Name, Book_Price,
             Language, publisher_fk, author_fk, category_fk, Publish_date, Stocked_Books, Current_books)
              VALUES ('$book_Id','$fileName', '$filePath','$bookName','$bookPrice','$language'
              ,'$publisher','$author','$category','$publishDate','$stockedBook','$stockedBook')";
